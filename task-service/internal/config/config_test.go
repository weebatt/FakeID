...
func TestNew_Success(t *testing.T) {
	...
	os.Setenv("KAFKA_TOPIC", "tasks-test")
	os.Setenv("KAFKA_MAX_RETRIES", "6")
	os.Setenv("KAFKA_RETRY_DELAY", "2")
	os.Setenv("KAFKA_TIMEOUT", "7")
	...
	// Kafka
	assert.Equal(t, "kafka:9093", cfg.Kafka.Brokers)
	assert.Equal(t, "tasks-test", cfg.Kafka.Topic)
	assert.Equal(t, 6, cfg.Kafka.MaxRetries)
	assert.Equal(t, 2, cfg.Kafka.RetryDelay)
	assert.Equal(t, 7, cfg.Kafka.Timeout)
...
func TestNew_DefaultValues(t *testing.T) {
	...
	assert.Equal(t, "tasks", cfg.Kafka.Topic)
	assert.Equal(t, 5, cfg.Kafka.MaxRetries)
	assert.Equal(t, 3, cfg.Kafka.RetryDelay)
	assert.Equal(t, 5, cfg.Kafka.Timeout)
